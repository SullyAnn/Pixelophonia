      ...
      <div id="choicesDiv">
        <div class="choice" v-for="(choice, index) in choices" :key="choice.id">
          ...
          <a
            :id="index"
            v-if="index == 0"
            @click="blur(choice, index)"
            v-on:click="sendChoice(index)"
            alt="image test"
          >
          ...
          <a
            ...
            alt="image test"
          >
            <img :src="require('assets/images/' + data.img)" /></a
          >-->
        </div>
      </div>
    </client-only>
...
export default {
  ...
  beforeMount() {
    ...
    socket.on("display-player-choice", (choice) => {
      console.log(
        "maintenant on est dans le display-player-choice du client " +
          choice.yourchoice
      );
      ...
      if (choice.yourchoice == 0)
        document.getElementById("0").style.filter = "grayscale(1) blur(5px)";
      else if (choice.yourchoice == 1)
        document.getElementById("1").style.filter = "grayscale(1) blur(5px)";
